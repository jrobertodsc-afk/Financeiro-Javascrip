...
class AbaHistStatus(BaseTab):



    # ==========================================================================

    # ÔöÇÔöÇ ABA 4: HIST├ôRICO DE STATUS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    def build(self, parent, bg, surface, border, accent, green, yellow, text, muted):
        ...
        """Aba que lista todos os logs de execu├º├úo anteriores do rob├┤."""
        ...
        ctk.CTkLabel(hdr, text="­ƒô£ HIST├ôRICO DE EXECU├ç├òES", font=("Segoe UI", 16, "bold"), text_color=accent).pack(side="left")

        ctk.CTkLabel(parent, text="Todos os logs de processamento salvos automaticamente pelo rob├┤.", font=("Segoe UI", 11), text_color=muted).pack(anchor="w", padx=25, pady=(0, 15))



        # ÔöÇÔöÇ FRAME PRINCIPAL ÔöÇÔöÇ
        ...
        # ÔöÇÔöÇ LISTA DE LOGS (esquerda) ÔöÇÔöÇ
        ...
        ctk.CTkLabel(frame_lista, text="SESS├òES SALVAS", font=("Segoe UI", 10, "bold"), text_color=accent).pack(pady=(15, 10))
        ...
        # ÔöÇÔöÇ DETALHE DO LOG (direita) ÔöÇÔöÇ
        ...
        ctk.CTkButton(
            ...
            barra_det, text="­ƒùæ APAGAR LOG", font=("Segoe UI", 10, "bold"), 
        ...
        ctk.CTkButton(
            ...
            barra_inf, text="­ƒöä ATUALIZAR LISTA", font=("Segoe UI", 11, "bold"), 
        ...
        ctk.CTkButton(
            ...
            barra_inf, text="­ƒùæ LIMPAR TUDO", font=("Segoe UI", 11, "bold"), 
    ...
    def _carregar_lista_logs(self):
        ...
        try:
            ...
            arquivos = sorted(
                ...
                [f for f in os.listdir(config.PASTA_ATUAL) if f.startswith("log_") and f.endswith(".txt")],
        ...
        for arq in arquivos:
            ...
            caminho = os.path.join(config.PASTA_ATUAL, arq)
            ...
            self._hist_log_listbox.insert("end", f" ­ƒôä {label}")
        ...
        self._hist_lbl_total.configure(text=f"{total} SESS├òES ENCONTRADAS")
    ...
    def _carregar_log_selecionado(self, event=None):
        ...
        """Exibe o conte├║do do log selecionado na listbox."""
        ...
        try:
            ...
            ok    = conteudo.count("Ô£à")

            erros = conteudo.count("ÔØî")

            dup   = conteudo.count("ÔÖ╗´©Å")

            resumo_txt = f"Ô£à {ok} OK   ÔØî {erros} ERROS   ÔÖ╗´©Å {dup} DUP   ÔÇö   {os.path.basename(caminho)}"
    ...
    def _carregar_lista_logs(self):
        ...
        for arq in arquivos:
            ...
            # Tenta extrair resumo (├║ltima linha do arquivo)
    ...
    def _carregar_log_selecionado(self, event=None):
        ...
        try:
            ...
            resumo_txt = f"Ô£à {ok} OK   ÔØî {erros} Erro(s)   ÔÖ╗´©Å {dup} Dup.   ÔÇö   {os.path.basename(caminho)}"
