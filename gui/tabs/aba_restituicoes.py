...
class AbaRestituicoes(BaseTab):
    def build(self, parent, bg, surface, border, accent, green, yellow, text, muted):
        ...
        ctk.CTkButton(
            ...
            width=130, height=30, command=lambda: os.startfile(config.PASTA_INPUT_SMART)
        ...
        ctk.CTkButton(
            ...
            width=130, height=30, command=lambda: os.startfile(config.PASTA_DOSSIES)
        ).pack(side="left", padx=5)
        ...
        self.card_restituido = self._criar_card_roi(cards_f, "💰 RESTITUIÇÃO LOCALIZADA", "R$ 0,00", accent, "#141414", accent)
        self.card_restituido.pack(side="left", fill="both", expand=True, padx=(0, 10))
        
        # Card 2: Biblioteca
        self.card_docs = self._criar_card_roi(cards_f, "📚 NOTAS VINCULADAS", "0 notas", "#FFFFFF", "#141414", border)
        self.card_docs.pack(side="left", fill="both", expand=True, padx=5)
        
        # Card 3: Eficiência
        self.card_status = self._criar_card_roi(cards_f, "🚀 STATUS OPERACIONAL", "Sistema Ocioso", yellow, "#141414", border)
        self.card_status.pack(side="left", fill="both", expand=True, padx=(10, 0))

        # ── MONITOR DE ATIVIDADE ──
        frame_work = ctk.CTkFrame(parent, fg_color="#0A0A0A", corner_radius=12, border_width=1, border_color="#141414")
    ...
    def _criar_card_roi(self, parent, titulo, valor, color, bg, border):
        ...
        ctk.CTkLabel(f, text=titulo, font=("Segoe UI", 10, "bold"), text_color="#141414").pack(pady=(20, 0))
    ...
    def _processar_dossies_thread(self):
        ...
        try:
            pasta_xmls = config.PASTA_RESTITUICOES_XML
            notas_devolucao = []
            ...
            for nf in notas_devolucao:
                ...
                uf_folder = os.path.join(config.PASTA_BIBLIOTECA_GNRE, nf['uf_destino'])
            ...
            if dossies_gerados:
                ...
                zip_path = os.path.join(config.PASTA_DOSSIES, zip_name)
