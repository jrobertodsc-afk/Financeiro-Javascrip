...
from gui.design_system import COLORS, FONTS, create_card, create_input, create_combo, create_btn, create_label

class AbaRecebiveis(BaseTab):
    ...
    def build(self, parent, bg, surface, border, accent, green, yellow, text, muted):
        ...
        create_label(hdr, "💳 RECEBÍVEIS & INTEGRAÇÕES", type="h2", text_color=COLORS["accent"]).pack(side="left")
        create_label(parent, "Consulte agenda de recebíveis e vendas em tempo real. Configure as credenciais nas CONFIGURAÇÕES.", type="muted").pack(anchor="w", padx=25, pady=(0, 15))
    ...
    def _card_credencial(self, parent, bg, surface, muted, text, accent,
        ...
        create_label(frame, titulo, type="label", text_color=COLORS["accent"]).grid(row=0, column=0, columnspan=4, sticky="w", pady=(0, 6), padx=14)
        ...
        status_cor = COLORS["success"] if ok else COLORS["warning"]
        create_label(frame, status_txt, type="caption", text_color=status_cor).grid(row=1, column=0, columnspan=4, sticky="w", pady=(0, 8), padx=14)

        for i, (label, var_key, default, show) in enumerate(campos):
            row = 2 + i
            create_label(frame, label, type="caption", text_color=COLORS["muted"]).grid(row=row, column=0, sticky="w", padx=(14, 6))
            var = tk.StringVar(value=default)
            var_dict[var_key] = var
            e = create_input(frame, width=300, show="*" if show else "")
    ...
    def _resultado_box(self, parent, bg, height=10):
        ...
        box = scrolledtext.ScrolledText(
            ...
            bg="#0a0a0a", fg="#a0c4ff", relief="flat", bd=0,
    ...
    def _build_sub_pagarme(self, parent, bg, surface, accent, green, yellow, text, muted):
        ...
        # Credencial
        status_frame = self._build_status_frame(parent_frame, "Pagar.me API", bg, surface, text, muted,
                                                status_cfg=[config.PAGARME_API_KEY],
            campos=[
                ("API Key:", "api_key", config.PAGARME_API_KEY, True),
            ],
        ...
        self._pm_de = tk.Entry(filtros, font=("Segoe UI", 9), bg="#0a0a0a", fg=text,
        ...
        self._pm_ate = tk.Entry(filtros, font=("Segoe UI", 9), bg="#0a0a0a", fg=text,
                                 insertbackground=accent, relief="flat", bd=3, width=12)
        ...
        tk.Button(filtros, text="🔍 Consultar Agenda",
                  font=("Segoe UI", 9, "bold"), bg="#ffffff", fg="white",
    ...
    def _consultar_pagarme(self):
        if not self._checar_requests():
            self._log_resultado(self._pm_resultado, "❌ Biblioteca 'requests' não instalada. Execute: pip install requests", "#a1a1aa")
            return

        api_key = self._pm_vars.get("api_key", tk.StringVar()).get().strip()
        if not api_key:
            self._log_resultado(self._pm_resultado, "⚠️ Informe a API Key do PagarMe.", "#ffffff")
            return
        ...
        try:
            ...
            self._log_resultado(self._pm_resultado, "⚠️ Data inválida. Use DD/MM/AAAA.", "#ffffff")
            return
        ...
        def consultar():
            try:
                ...
                resp = requests.get(f"{config.PAGARME_API_URL}/payables", headers=headers,
                                    params=params, timeout=15)

                if resp.status_code == 401:
                    self.root.after(0, lambda: self._log_resultado(
                        self._pm_resultado, "❌ API Key inválida ou sem permissão.", "#a1a1aa"))
                    return
                if resp.status_code != 200:
                    self.root.after(0, lambda: self._log_resultado(
                        self._pm_resultado, f"❌ Erro {resp.status_code}: {resp.text[:200]}", "#a1a1aa"))
                    return
                ...
                if not itens:
                    self.root.after(0, lambda: self._log_resultado(
                        self._pm_resultado, "ℹ️ Nenhum recebível encontrado no período.", "#ffffff"))
                    return
                ...
                self.root.after(0, lambda: self._log_resultado(
                    self._pm_resultado, f"❌ Erro: {e}", "#a1a1aa"))
    ...
    def _saldo_pagarme(self):
        if not self._checar_requests():
            self._log_resultado(self._pm_resultado, "❌ 'requests' não instalado.", "#a1a1aa")
            return

        api_key = self._pm_vars.get("api_key", tk.StringVar()).get().strip()
        if not api_key:
            self._log_resultado(self._pm_resultado, "⚠️ Informe a API Key.", "#ffffff")
            return
        ...
        def consultar():
            try:
                ...
                resp = requests.get(f"{config.PAGARME_API_URL}/balance", headers=headers, timeout=15)
                ...
                def atualizar():
                    self._log_resultado(self._pm_resultado,
                        ...
                        f"   Total transferido:     R$ {transf:>12,.2f}", "#ffffff")
    ...
    def _build_sub_rede(self, parent, bg, surface, accent, green, yellow, text, muted):
        ...
        tk.Label(info, text="🔴  Rede — Vendas & Recebíveis por Filial",
                 font=("Segoe UI", 10, "bold"), fg="#a1a1aa", bg=surface).pack(anchor="w")
        tk.Label(info,
        ...
        self._card_credencial(
            ...
            status_cfg=[config.REDE_CLIENT_ID, config.REDE_CLIENT_SECRET, config.REDE_PV],
            campos=[
                ("Client ID:", "client_id", config.REDE_CLIENT_ID, False),
                ("Client Secret:", "client_secret", config.REDE_CLIENT_SECRET, True),
                ("PV (estabelecimento):", "pv", config.REDE_PV, False),
            ],
        ...
        self._rede_data = tk.Entry(filtros, font=("Segoe UI", 9), bg="#0a0a0a", fg=text,
                                    insertbackground=accent, relief="flat", bd=3, width=12)
        ...
        tk.Button(filtros, text="📊 Consultar Vendas do Dia",
                  font=("Segoe UI", 9, "bold"), bg="#a1a1aa", fg="white",
                  relief="flat", bd=0, padx=12, pady=5, cursor="hand2",
    ...
    def _rede_obter_token(self):
        ...
        resp = requests.post(
            f"{config.REDE_API_URL}/v1/token",
    ...
    def _consultar_rede_vendas(self):
        if not self._checar_requests():
            self._log_resultado(self._rede_resultado, "❌ 'requests' não instalado. Execute: pip install requests", "#a1a1aa")
            return
        ...
        def consultar():
            try:
                ...
                import requests
                resp = requests.get(
                    f"{config.REDE_API_URL}/v1/transactions",
                ...
                if resp.status_code != 200:
                    self.root.after(0, lambda: self._log_resultado(
                        self._rede_resultado, f"❌ Erro {resp.status_code}: {resp.text[:300]}", "#a1a1aa"))
                    return
                ...
                if not transacoes:
                    self.root.after(0, lambda: self._log_resultado(
                        self._rede_resultado, "ℹ️ Nenhuma venda encontrada.", "#ffffff"))
                    return
                ...
                self.root.after(0, lambda: self._log_resultado(
                    self._rede_resultado, f"❌ Erro: {e}", "#a1a1aa"))
    ...
    def _consultar_rede_recebiveis(self):
        if not self._checar_requests():
            self._log_resultado(self._rede_resultado, "❌ 'requests' não instalado.", "#a1a1aa")
            return
        self._limpar_resultado(self._rede_resultado)
        ...
        def consultar():
            try:
                ...
                resp = requests.get(
                    f"{config.REDE_API_URL}/v1/receivables",
                    headers={"Authorization": f"Bearer {token}"},
                    params=params, timeout=15
                )
                if resp.status_code != 200:
                ...
                if not recebiveis:
                    self.root.after(0, lambda: self._log_resultado(
                        self._rede_resultado, "ℹ️ Nenhum recebível encontrado.", "#ffffff"))
                    return
    ...
    def _build_sub_getnet(self, parent, bg, surface, accent, green, yellow, text, muted):
        ...
        tk.Label(info, text="🟢  Getnet — Vendas & Recebíveis",
                 font=("Segoe UI", 10, "bold"), fg="#ffffff", bg=surface).pack(anchor="w")
        tk.Label(info,
        ...
        self._card_credencial(
            ...
            status_cfg=[config.GETNET_CLIENT_ID, config.GETNET_CLIENT_SECRET, config.GETNET_SELLER_ID],
            campos=[
                ("Client ID:", "client_id", config.GETNET_CLIENT_ID, False),
                ("Client Secret:", "client_secret", config.GETNET_CLIENT_SECRET, True),
                ("Seller ID:", "seller_id", config.GETNET_SELLER_ID, False),
            ],
        ...
        filtros.pack(fill="x", pady=(0, 8))
        tk.Label(filtros, text="Data:", fg=muted, bg=surface, font=("Segoe UI", 8)).grid(row=0, column=0, padx=(0, 4))
        self._getnet_data = tk.Entry(filtros, font=("Segoe UI", 9), bg="#0a0a0a", fg=text,
                                      insertbackground=accent, relief="flat", bd=3, width=12)
    ...
    def _getnet_obter_token(self):
        ...
        cred = base64.b64encode(f"{client_id}:{client_secret}".encode()).decode()
        resp = requests.post(
            f"{config.GETNET_API_URL}/auth/oauth/v2/token",
            headers={"Authorization": f"Basic {cred}",
                     "Content-Type": "application/x-www-form-urlencoded"},
    ...
    def _consultar_getnet_vendas(self):
        if not self._checar_requests():
            self._log_resultado(self._getnet_resultado, "❌ 'requests' não instalado.", "#a1a1aa")
            return
        ...
        def consultar():
            try:
                ...
                import requests
                resp = requests.get(
                    f"{config.GETNET_API_URL}/v1/reports/sales/transactions",
                    headers={"Authorization": f"Bearer {token}",
                             "seller_id": seller_id},
                    params={
                ...
                if resp.status_code != 200:
                    self.root.after(0, lambda: self._log_resultado(
                        self._getnet_resultado, f"❌ Erro {resp.status_code}: {resp.text[:300]}", "#a1a1aa"))
                    return
                ...
                if not vendas:
                    self.root.after(0, lambda: self._log_resultado(
                        self._getnet_resultado, "ℹ️ Nenhuma venda encontrada.", "#ffffff"))
                    return
                ...
                self.root.after(0, lambda: self._log_resultado(
                    self._getnet_resultado, f"❌ Erro: {e}", "#a1a1aa"))
    ...
    def _consultar_getnet_recebiveis(self):
        ...
        def consultar():
            try:
                ...
                resp = requests.get(
                    f"{config.GETNET_API_URL}/v1/reports/receivables",
                ...
                if not recebiveis:
                    self.root.after(0, lambda: self._log_resultado(
                        self._getnet_resultado, "ℹ️ Nenhum recebível encontrado.", "#ffffff"))
                    return
    ...
    def _build_sub_microvix(self, parent, bg, surface, accent, green, yellow, text, muted):
        ...
        self._card_credencial(
            ...
            status_cfg=[config.MICROVIX_CNPJ, config.MICROVIX_USUARIO, config.MICROVIX_CHAVE],
            campos=[
                ("CNPJ (só números):", "cnpj",    config.MICROVIX_CNPJ,    False),
                ("Usuário:",           "usuario",  config.MICROVIX_USUARIO, False),
                ("Senha:",             "senha",    config.MICROVIX_SENHA,   True),
                ("Chave da Empresa:",  "chave",    config.MICROVIX_CHAVE,   True),
            ],
        ...
        filtros.pack(fill="x", pady=(0, 8))
        tk.Label(filtros, text="Data:", fg=muted, bg=surface, font=("Segoe UI", 8)).grid(row=0, column=0, padx=(0, 4))
        self._mv_data = tk.Entry(filtros, font=("Segoe UI", 9), bg="#0a0a0a", fg=text,
                                  insertbackground=accent, relief="flat", bd=3, width=12)
        ...
        self._mv_filial = tk.Entry(filtros, font=("Segoe UI", 9), bg="#0a0a0a", fg=text,
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    ...
    def _consultar_microvix_vendas(self):
        if not self._checar_requests():
            self._log_resultado(self._mv_resultado, "❌ 'requests' não instalado.", "#a1a1aa")
            return
            ...
            try:
                ...
                def atualizar():
                    ...
                    for fil, val in sorted(por_filial.items()):
                        nome_fil = config.DE_PARA_FILIAL.get(f"Filial_{fil}", fil)
                        self._log_resultado(self._mv_resultado,
                ...
                self.root.after(0, lambda: self._log_resultado(
                    self._mv_resultado, f"❌ Erro: {e}", "#a1a1aa"))
            ...
            try:
                ...
                def atualizar():
                    self._log_resultado(self._mv_resultado,
                        ...
                        f"   Valor em estoque:   R$ {valor_estoque:>12,.2f}", "#ffffff")
