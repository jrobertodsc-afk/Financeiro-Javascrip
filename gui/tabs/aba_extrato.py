...
class AbaExtrato(BaseTab):
    ...
    def build(self, parent, bg, surface, border, accent, green, yellow, text, muted):
        ...
        ctk.CTkButton(frame_bts, text="⚡ GERAR FLUXO CONSOLIDADO", font=("Segoe UI", 12, "bold"), fg_color="#ffffff", hover_color="#ffffff", text_color="white", height=45, width=250, command=self._processar_fluxo_consolidado).pack(side="left", padx=(0, 15))
        ...
        self.extrato_email.insert(0, config.EMAIL_DESTINO_EXTRATO)
        self.extrato_email.pack(side="left", pady=10)
        
        ctk.CTkButton(frame_email, text="📤 ENVIAR POR E-MAIL", font=("Segoe UI", 10, "bold"), fg_color="#141414", text_color="#ffffff", height=35, width=180, command=self._enviar_dashboard_email).pack(side="left", padx=15)
        ...
        self.extrato_tree.tag_configure("deb",  background="#a1a1aa", foreground="#a1a1aa")
        self.extrato_tree.tag_configure("cred", background="#152d1f", foreground="#ffffff")
        self.extrato_tree.tag_configure("getnet",background="#0a0a0a", foreground="#ffffff")
    ...
    def _processar_extrato(self):
        ...
        if not caminho or not os.path.exists(caminho):
            self.extrato_status.config(text="⚠️ Selecione um arquivo PDF válido.", fg="#ffffff")
            return

        self.extrato_status.config(text="⏳ Lendo extrato...", fg="#ffffff")
            ...
            try:
                ...
                if not contas:
                    self.root.after(0, lambda: self.extrato_status.config(
                        text="⚠️ Nenhuma conta encontrada no PDF.", fg="#ffffff"))
                    return
                ...
                caminho_excel = os.path.join(config.PASTA_EXTRATOS, nome_excel)
                ...
                if ok:
                    ...
                    try:
                        import subprocess
                        os.makedirs(config.PASTA_BACKUP_EXTRATOS, exist_ok=True) if not os.path.exists(config.PASTA_BACKUP_EXTRATOS) else None
                        subprocess.run(
                            ["robocopy", config.PASTA_EXTRATOS, config.PASTA_BACKUP_EXTRATOS, "/XO", "/NP", "/LOG+:NUL"],
    ...
    def _processar_fluxo_consolidado(self):
        ...
        if not paths_getnet and not path_itau:
            self.extrato_status.config(
                text="⚠️ Selecione ao menos um arquivo (extrato Itaú ou Getnet).", fg="#ffffff")
            return

        self.extrato_status.config(text="⏳ Processando...", fg="#ffffff")
        self.root.update_idletasks()
        ...
        def processar():
            try:
                ...
                caminho_excel = os.path.join(config.PASTA_EXTRATOS, nome_arq)
                ...
                self.root.after(0, lambda: self.extrato_status.config(text=msg, fg="#ffffff"))
                if ok:
                    os.startfile(resultado)
    ...
    def _enviar_dashboard_email(self):
        ...
        if not destino:
            self.extrato_status.config(text="⚠️ Informe o e-mail de destino.", fg="#ffffff")
            return

        # Busca o Excel mais recente na pasta de extratos
        os.makedirs(config.PASTA_EXTRATOS, exist_ok=True)
        arquivos = [f for f in os.listdir(config.PASTA_EXTRATOS) if f.endswith('.xlsx')]
        if not arquivos:
            self.extrato_status.config(text="⚠️ Nenhum dashboard gerado ainda. Processe o extrato primeiro.", fg="#ffffff")
            return

        ultimo = max(arquivos, key=lambda f: os.path.getmtime(os.path.join(config.PASTA_EXTRATOS, f)))
        caminho_excel = os.path.join(config.PASTA_EXTRATOS, ultimo)

        self.extrato_status.config(text=f"⏳ Enviando dashboard para {destino}...", fg="#ffffff")
        self.root.update_idletasks()
        ...
        def enviar():
            try:
                ...
                <div style='background:#0a0a0a;padding:20px;border-radius:8px;'>
                  <h2 style='color:#ffffff;margin:0;'>📊 Dashboard Contas a Pagar</h2>
                  <p style='color:#a1a1aa;'>Gerado em {datetime.now().strftime('%d/%m/%Y às %H:%M')}</p>
                </div>
                ...
                self.root.after(0, lambda: self.extrato_status.config(
                    text=f"✅ Dashboard enviado para {destino}!", fg="#ffffff"))
            except Exception as e:
                self.root.after(0, lambda: self.extrato_status.config(
                    text=f"❌ Erro ao enviar: {e}", fg="#a1a1aa"))
