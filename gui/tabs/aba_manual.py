...
from services.database import salvar_pagamentos_autorizados, listar_pagamentos_autorizados


class AbaManual(BaseTab):
    """
    ...
    def _man_build_form(self, parent,
        ...
        cats = getattr(__import__('config'), 'config.CATEGORIAS_LISTA', [
    ...
    def _man_build_historico(self, parent,
        ...
        # Tags de linha
        self._man_tree.tag_configure("par",   background="#0a0a0a", foreground="#e5e5e5")
        self._man_tree.tag_configure("impar", background="#0a0a0a", foreground="#e5e5e5")
        self._man_tree.tag_configure("sel",   background="#141414", foreground="#CCFF00")
    ...
    def _man_auto_classificar(self, event=None):
        ...
        def classificar():
            try:
                resp, cat, desc = auto_classificar(nome, 0, cnpj)
                if cat and cat != "A CLASSIFICAR":
                    ...
                    if desc:
                        self.after(0, lambda: self._man_desc.delete(0, "end"))
                        self.after(0, lambda: self._man_desc.insert(0, desc))
                    ...
                    self.after(0, lambda: self._man_lbl_class.configure(
                        ...
                        text_color=self._man_colors.get("yellow", "#ffffff")
                    ))
            ...
            try:
                ...
                def ok():
                    self._man_lbl_status.configure(
                        ...
                        text_color=self._man_colors.get("green", "#ffffff")
                    )
                    self._man_btn_salvar.configure(
                ...
                def err():
                    self._man_lbl_status.configure(
                        text=f"❌ Erro: {e}",
                        text_color=self._man_colors.get("error", "#a1a1aa") if "error" in self._man_colors else "#a1a1aa"
                    )
                    self._man_btn_salvar.configure(
                        state="normal", text="✅ LANÇAR E APROVAR"
                    )
    ...
    def _man_menu_contexto(self, event):
        ...
        menu = tk.Menu(self.root, tearoff=0, bg="#0a0a0a", fg="#e5e5e5",
                       activebackground="#141414", activeforeground="#CCFF00",
                       font=("Segoe UI", 10))
