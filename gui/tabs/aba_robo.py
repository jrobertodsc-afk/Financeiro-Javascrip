...
class AbaRobo(BaseTab):
    ...
    def build(self, parent, bg, surface, border, accent, green, yellow, text, muted):
        ...
        ctk.CTkLabel(f3, text=f"...{config.PASTA_ATUAL[-20:]}", font=("Segoe UI", 10, "bold"), text_color=text).pack(anchor="w")
    ...
    def _modern_stat_card(self, parent, title, value, color, column):
        card = ctk.CTkFrame(parent, fg_color="#141414", corner_radius=12, border_width=1, border_color="#141414")
        card.grid(row=0, column=column, sticky="nsew", padx=10)
        
        ctk.CTkLabel(card, text=title, font=("Segoe UI", 10, "bold"), text_color="#141414").pack(pady=(15, 0))
        
        val_lbl = ctk.CTkLabel(card, text=value, font=("Segoe UI", 32, "bold"), text_color=color)
        val_lbl.pack(pady=(5, 10))
        
        ctk.CTkLabel(card, text="TOTAL ITEMS", font=("Segoe UI", 8), text_color="#141414").pack(pady=(0, 15))
    ...
    def _contar_pdfs(self):
        try:
            return len([f for f in os.listdir(config.PASTA_ENTRADA) if f.lower().endswith('.pdf')])
        except:
            return 0
    def _abrir_saida(self):
        os.makedirs(config.PASTA_SAIDA, exist_ok=True)
        os.startfile(config.PASTA_ATUAL)
