...
class AbaBusca(BaseTab):
    ...
    def build(self, parent, bg, surface, border, accent, green, yellow, text, muted):
        ...
        create_label(linha1, "NOME DO FORNECEDOR", type="label", text_color=COLORS["muted"]).grid(row=0, column=0, sticky="w", padx=(0, 20))
        self.busca_nome = create_input(linha1, width=280, placeholder_text="Ex: COELBA...")
        self.busca_nome.grid(row=1, column=0, padx=(0, 20), pady=(5, 0))
        self.busca_nome.bind("<Return>", lambda e: self._executar_busca())

        create_label(linha1, "CATEGORIA", type="label", text_color=COLORS["muted"]).grid(row=0, column=1, sticky="w", padx=(0, 20))
        
        self.busca_cat_var = tk.StringVar(value="Todas")
        self.busca_cat = create_combo(
            linha1, 
        ...
        create_label(linha1, "EMPRESA", type="label", text_color=COLORS["muted"]).grid(row=0, column=2, sticky="w")
        self.busca_empresa_var = tk.StringVar(value="Todas")
        self.busca_empresa = create_combo(
            linha1, 
        ...
        create_label(linha2, "PERÍODO (DE / ATÉ)", type="label", text_color=COLORS["muted"]).grid(row=0, column=0, columnspan=2, sticky="w", padx=(0, 20))
        
        self.busca_data_de = create_input(linha2, width=130)
        ...
        self.busca_data_ate = create_input(linha2, width=130)
        ...
        create_label(linha2, "VALOR (MÍN / MÁX)", type="label", text_color=COLORS["muted"]).grid(row=0, column=2, columnspan=2, sticky="w")
        
        self.busca_val_min = create_input(linha2, placeholder_text="0,00", width=110)
        self.busca_val_min.grid(row=1, column=2, padx=(0, 10), pady=(5, 0))

        self.busca_val_max = create_input(linha2, placeholder_text="999.999", width=110)
        ...
        self.btn_buscar = create_btn(acoes_f, text="🔍 EXECUTAR BUSCA", width=200, height=45)
        self.btn_buscar.pack(side="left")

        create_btn(acoes_f, text="🔄 Reclassificar Base", style="secondary", width=180, height=45, command=self._reclassificar_comprovantes).pack(side="left", padx=15)

        self.lbl_resultado = create_label(acoes_f, "Aguardando parâmetros...", type="muted")
        self.lbl_resultado.pack(side="right")

        # ── RESULTADO (Treeview estilizada) ──
        tree_container = ctk.CTkFrame(parent, fg_color=COLORS["card"], corner_radius=12, border_width=1, border_color=COLORS["border"])
        ...
        # ── RODAPÉ ──
        rod = ctk.CTkFrame(parent, fg_color=COLORS["surface"], height=60, corner_radius=0)
        rod.pack(fill="x", side="bottom")

        create_label(rod, "E-MAIL DESTINO:", type="label", text_color=COLORS["muted"]).pack(side="left", padx=(25, 10))
        self.busca_email_destino = create_input(rod, width=250)
        self.busca_email_destino.insert(0, config.EMAIL_DESTINO_DP)
        self.busca_email_destino.pack(side="left", padx=(0, 15))

        create_btn(rod, "📤 ENVIAR SELECIONADOS", style="secondary", width=180, height=32, command=self._enviar_selecionados).pack(side="left", padx=5)
        create_btn(rod, "💾 DOWNLOAD", style="secondary", width=120, height=32, command=self._baixar_selecionados).pack(side="left", padx=5)
        create_btn(rod, "📋 AUTORIZAÇÃO", style="danger", width=140, height=32, command=self._enviar_para_autorizacao).pack(side="left", padx=5)

        self.lbl_status_envio = create_label(rod, "", type="label", text_color=COLORS["success"])
    ...
    def _executar_busca(self):
        ...
        for root_dir, _, files in os.walk(config.PASTA_SAIDA):
            partes = root_dir.replace(config.PASTA_SAIDA, "").split(os.sep)
    ...
    def _baixar_selecionados(self):
        ...
        if not itens:
            self.lbl_status_envio.config(
                text="⚠️ Nenhum comprovante selecionado.", fg="#ffffff")
            return
    ...
    def _reclassificar_comprovantes(self):
        ...
        self.lbl_resultado.config(text="⏳ Reclassificando...", fg="#ffffff")
                ...
                for root_dir, _, files in os.walk(config.PASTA_SAIDA):
                    for f in files:
                        ...
                        try:
                            ...
                            resp_novo, cat_nova, desc_nova = auto_classificar(nome, "")
    ...
    def _enviar_selecionados(self):
        ...
        if not itens:
            self.lbl_status_envio.config(text="⚠️ Nenhum comprovante para enviar.", fg="#ffffff")
            return

        destino = self.busca_email_destino.get().strip()
        if not destino:
            self.lbl_status_envio.config(text="⚠️ Informe o e-mail de destino.", fg="#ffffff")
            return

        self.lbl_status_envio.config(text=f"⏳ Enviando {len(itens)} comprovante(s) para {destino}...", fg="#ffffff")
        self.root.update_idletasks()
        ...
        def enviar():
            try:
                ...
                self.root.after(0, lambda: self.lbl_status_envio.config(
                    text=f"✅ E-mail enviado! {anexados} PDF(s) para {destino}", fg="#ffffff"))
    ...
    def _abrir_dialogo_periodo(self):
        ...
        tk.Label(win, text="📧 Reenviar Relatório RH — Comprovantes",
                 font=("Segoe UI", 12, "bold"), fg="#ffffff", bg="#0a0a0a").pack(pady=(16, 2))
        tk.Label(win, text="Escolha o período. Para um dia específico, preencha De e Até com a mesma data.",
                 font=("Segoe UI", 8), fg="#a1a1aa", bg="#0a0a0a", wraplength=380).pack(pady=(0, 10))
        ...
        # ── Modo de filtro ──
        frame_modo = tk.Frame(win, bg="#0a0a0a")
        ...
        tk.Radiobutton(frame_modo, text="Intervalo de datas",
                       ...
                       fg=text, bg="#0a0a0a", selectcolor="#0a0a0a",
                       activebackground="#0a0a0a", font=("Segoe UI", 9),
        ...
        de_entry = tk.Entry(frame_int, textvariable=de_var, font=("Segoe UI", 10),
                            bg="#0a0a0a", fg=text, insertbackground=accent,
        ...
        tk.Button(frame_atalhos, text="Hoje",
                  font=("Segoe UI", 7), bg="#27272a", fg=text, relief="flat",
        ...
        # ── E-mail destino ──
        frame_email = tk.Frame(win, bg="#0a0a0a")
        frame_email.pack(fill="x", padx=20, pady=(6, 0))
        tk.Label(frame_email, text="📧 Enviar para:", fg=muted, bg="#0a0a0a",
                 font=("Segoe UI", 8)).pack(side="left")
        email_var = tk.StringVar(value=config.EMAIL_DESTINO_DP)
        tk.Entry(frame_email, textvariable=email_var, font=("Segoe UI", 9),
                 bg="#141414", fg=text, insertbackground=accent,
            ...
            if modo_var.get() == "intervalo":
                try:
                    ...
                    tk.Label(win, text="⚠️ Data inválida. Use DD/MM/AAAA.",
                             fg="#ffffff", bg="#0a0a0a", font=("Segoe UI", 8)).pack()
                    return
    ...
    def _reenviar_relatorio_periodo_datas(self, dt_de, dt_ate, label, destino=None):
        ...
        if destino is None:
            destino = config.EMAIL_DESTINO_DP
        ...
        for root_dir, dirs, files in os.walk(config.PASTA_SAIDA):
            partes = root_dir.replace(config.PASTA_SAIDA, "").split(os.sep)
    ...
    def _reenviar_relatorio_periodo(self, mes, ano, mes_nome, destino=None):
        ...
        itens = []
        for empresa_dir in os.listdir(config.PASTA_SAIDA):
            caminho_empresa = os.path.join(config.PASTA_SAIDA, empresa_dir)
